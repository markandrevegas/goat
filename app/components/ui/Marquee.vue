...
const loopItems = computed(() => {
	...
	const items = (galleryData.value as GalleryItem[])
		// 1. Filter out responsive variant files generated by your script (-300w, -600w, etc.)
		.filter((img) => {
			const identifier = String(img.name || img.url || img.id || "")
			return !/-\d+w\.(webp|png|jpe?g)$/i.test(identifier)
		})
		// 2. Map only the base original files
		.map((img) => {
			...
			return {
				...
				label: img.name
					? img.name
							.replace(/-\d+w\.[^/.]+$/, "") // Remove -300w suffix if present
