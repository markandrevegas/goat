...
<template>
	<Transition enter-active-class="transition ease-out duration-300" enter-from-class="translate-y-full opacity-0" enter-to-class="translate-y-0 opacity-100" leave-active-class="transition ease-in duration-200" leave-from-class="translate-y-0 opacity-100" leave-to-class="translate-y-full opacity-0">
		<div v-if="showBanner" class="bg-palladian text-brand fixed inset-x-0 bottom-0 z-50 border-t border-gray-200 p-8 shadow-lg sm:p-6 text-sm" role="dialog" aria-live="polite" aria-label="Cookie consent">
			<div class="mx-auto max-w-4xl">
				<div v-if="!showDetails" class="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
					<div class="sm:max-w-xl">
						<p class="text-brand mb-4">Vi bruger cookies til at forbedre din oplevelse og analysere brugen af siden. Du kan acceptere alle cookies, afvise ikke-nødvendige cookies eller administrere dine præferencer.</p>
						<a @click="showDetails = true" class="font-semibold flex items-center gap-2">Indstillinger<span><OpenSettings :size="12" /></span></a>
					</div>

					<div class="mt-4 flex shrink-0 sm:mt-0 gap-4">
				...
				<div v-else class="flex flex-col gap-4">
					<div class="flex items-center justify-between">
						...
						<button type="button" class="text-gray-500 text-sm hover:text-gray-700" @click="showDetails = false">Back</button>
					</div>
					...
					<div class="space-y-3">
						<div class="flex items-start justify-between gap-4">
							<div>
								...
								<p class="text-gray-500 text-sm">Nødvendige for, at siden fungerer. Altid aktiveret.</p>
							</div>
						...
						<div class="flex items-start justify-between gap-4">
							<div>
								...
								<p class="text-gray-500 text-sm">Hjælper os med at forstå, hvordan siden bruges.</p>
							</div>
						...
						<div class="flex items-start justify-between gap-4">
							<div>
								...
								<p class="text-gray-500 text-sm">Bruges til personaliserede annoncer og kampagner..</p>
							</div>
					...
					<div class="flex justify-between sm:justify-end gap-2 py-8">
