...
async function exportData() {
	...
	try {
		...
		const [pages, posts] = await Promise.all([fetchEndpoint("pages"), fetchEndpoint("posts")])

		await Promise.all([fs.writeFile(`${OUT_DIR}/pages.json`, JSON.stringify(pages, null, 2)), fs.writeFile(`${OUT_DIR}/posts.json`, JSON.stringify(posts, null, 2))])
